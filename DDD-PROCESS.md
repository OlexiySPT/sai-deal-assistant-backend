...
## ***Reasons to use simplified approach***
  ...
  "items": [
    {
      ...
      "registrationDate": "2023-04-25T12:36:11.098Z"
    }
  ]
}
```
...
(`Osereda.Boilerplate.Persistence.EntityConfigurations.SampleCustomerConfiguration` in this case) :			
```
                        builder.Property(c => c.TaxNumber)
				.HasColumnType("varchar")
				.HasMaxLength(50);
			builder.Property(c => c.VatPayerNumber)
				.HasColumnType("varchar")
				.HasMaxLength(50);
...
Use UnitTests to debug instead of running app and inputing data via Swagger - it is easy and saves quite a bit of time for your .
...
```
Use SelectAsync to select tableColumns into the DTO.
...
## ***WebApi level classes:***
### **Controllers**
Controllers should be integration tested.
Integration tests are created in Tosca ApiScan tool and run manually by the QA against the real environment.
Probably we need to think about testing await Mediator.Send(comand) against mocked DB to be sure validation and handling work successfully together. But I`m not 100% sure about this.